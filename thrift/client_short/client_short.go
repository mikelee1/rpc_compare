	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			...
			for j := 0; j < m; j++ {
				...
				client := genpack.NewGreeterClientFactory(useTransport, protocolFactory)
				...
				t := time.Now().UnixNano()
				reply, err := client.Say(context.TODO(),args)
...
func prepareArgs() *genpack.BenchmarkMessage {
	b := true
	var i int32 = 100000
	var i64 int64 = 100000
	var s = "许多往事在眼前一幕一幕，变的那麼模糊"

	var args genpack.BenchmarkMessage
